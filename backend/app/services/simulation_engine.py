...
def run_simulation(
    ...
    # Initialize RNG once per run (not inside loop) for proper Monte Carlo variation
    # Each simulation path will draw different random values from this RNG
    ...
    for sim_idx in range(n_sims):
        ...
        for month in range(1, horizon_months + 1):
            ...
            if scenario_type == "job_loss":
                start_month = scenario_params.get("start_month", 1)
                duration_months = scenario_params.get("duration_months", 3)
                replacement_pct = scenario_params.get("unemployment_replacement_pct", 0.0)
                if start_month <= month < start_month + duration_months:
                    income = base_income * replacement_pct
            
            # Add stochastic variation to income (if not in job loss period with 0 replacement)
            if income > 0 and sigma_income > 0:
                # Draw income shock from normal distribution: N(0, sigma_income)
                # Applies multiplicative shock: income * (1 + shock)
                income_shock = rng.normal(0, sigma_income)
                income = income * (1 + income_shock)
                income = max(0, income)  # Clamp to non-negative
            
            # Variable expenses shock
            if sigma_variable > 0:
                # Draw expense shock from normal distribution: N(0, sigma_variable)
                # Applies multiplicative shock: variable * (1 + shock)
                variable_shock = rng.normal(0, sigma_variable)
                variable = variable * (1 + variable_shock)
                variable = max(0, variable)  # Clamp to non-negative
            
            # Rent increase
            if scenario_type == "rent_increase":
                start_month = scenario_params.get("start_month", 1)
                rent_delta = scenario_params.get("rent_delta", 0)
                if month >= start_month:
                    fixed = fixed + rent_delta
            
            # Rate shock (affects APR but for MVP we keep min_payment same)
            if scenario_type == "rate_shock":
                # For MVP, rate shock increases APR but min_payment stays constant
                # unless scenario specifies min_payment_increase
                min_payment_increase = scenario_params.get("min_payment_increase", 0)
                debt_min = debt_min + min_payment_increase
            
            # Decision scenario
            if scenario_type == "decision":
                one_time_cost = scenario_params.get("one_time_cost", 0)
                extra_monthly_payment = scenario_params.get("extra_monthly_payment", 0)
            ...
            if scenario_type == "expense_shock":
                shock_month = scenario_params.get("shock_month", 1)
                shock_amount = scenario_params.get("shock_amount", 0)
                shock_duration = scenario_params.get("shock_duration", 1)
                if shock_month <= month < shock_month + shock_duration:
                    fixed = fixed + (shock_amount / shock_duration if shock_duration > 1 else shock_amount)
